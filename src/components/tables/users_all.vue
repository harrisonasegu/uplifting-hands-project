...
import CreateUserForm from '@/components/modal/content/users_createUserForm.vue'
import ViewDetails from '@/components/modal/content/users_userDetails.vue'
import ActionComment from '@/components/modal/content/users_actionComment.vue'

// data table columns
import { allUsersDataTableColumn as dataTableColumns } from '@/table-columns/users_all'
...
const dataSource = userStore.getAllUsers
...
class DataTable {
  ...
  unblockUser = () => {
    ...
    userStore.UnblockAccount(payload)
      .then((response) => {
        // close modal
        this.closeModal()
        // loading state
  ...
  handleDataTableButtonClick = (isButtonClicked: boolean) => {
    ...
    switch (dataTableClickedButtonProp.buttonClicked) {
      ...
      case 'reactivate user':
        // update modal prop
        modalAttrs.modalTitle = 'Reactivate User'
        modalAttrs.maskClosable = false
      ...
      case 'unblock user':
        ...
        modalAttrs.negativeText = 'Close'
        modalAttrs.positiveText = 'Unblock user'
...
<template>
    ...
    <div class="btn-and-search-container">
      ...
      <div class="btn-container">
        <n-button
          type="primary"
          :disabled="isLoading || loading"
          @click="dataTable.handleAddUserButtonClicked"
        >
  ...
  >
    <template #modalContent>
      <CreateUserForm
        v-if="modalAttrs.modalTitle === 'New User Creation'"
        @closeProductForm="dataTable.closeModal"
      />
      ...
      <ActionComment
        v-if="[
          'reactivate user', 'deactivate user', 'disapprove user'
