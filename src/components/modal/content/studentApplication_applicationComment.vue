...
function reviewApplication() {
  ...
  const payload = {
    ...
    review_Comments: model.value
  }

  // proceed to login
  fundOrFinancialAidStore.ReviewApplication(props.currentTab as number, payload)
    .then(response => {
...
function approveApplication() {
  ...
  const data = dataTableClickedButtonProp.data as FundApplication_Interface

  const payload = {
    applicationId: data.applicationId,
    approval_Comments: model.value
  }

  // proceed to login
  fundOrFinancialAidStore.ApproveApplication(props.currentTab as number, payload)
    .then(response => {
      // show notification for login error
      if (response.responseCode === '00') {
        // clear input
...
function declineApplication() {
  ...
  const payload = {
    applicationId: data.applicationId,
    reasons_For_Decline: model.value
  }

  // proceed to login
  fundOrFinancialAidStore.DeclineApplication(props.currentTab as number, payload)
    .then(response => {
      // show notification for login error
      if (response.responseCode === '00') {
        // clear input
